...
from .extract import extract_retail_price_rental_index
    ...
    def run(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not price_path.exists() or not rent_path.exists():
                raise
            meta_price = {
                "last_modified": state.get("last_modified_price"),
                "etag": state.get("etag_price"),
                "content_length": state.get("content_length_price"),
                "final_url": state.get("final_url_price") or self.PRICE_INDEX_URL,
                "downloaded_at_utc": state.get("downloaded_at_utc_price"),
            }
            meta_rent = {
                "last_modified": state.get("last_modified_rent"),
                "etag": state.get("etag_rent"),
                "content_length": state.get("content_length_rent"),
                "final_url": state.get("final_url_rent") or self.RENT_INDEX_URL,
                "downloaded_at_utc": state.get("downloaded_at_utc_rent"),
            }
            download_note = "Used existing local PDFs because one of the source files was locked."
        ...
        df_new = extract_retail_price_rental_index(price_path, rent_path)
        ...
        sql_path = Path(__file__).parent / "ed_retail_price_rental_index.sql"
        sync_cols = [
            "total_price_index",
            "total_rent_index",
            "athens_price_index",
        ...
        def shape_output(df: pd.DataFrame) -> pd.DataFrame:
            ...
            shaped = shaped.sort_values(["Year", "Year Half"]).reset_index(drop=True)
