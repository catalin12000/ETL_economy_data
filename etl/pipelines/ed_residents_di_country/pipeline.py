...
from .extract import extract_residents_di_country
...
class Pipeline:
    ...
    def run(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        sql_path = Path(__file__).parent / "ed_residents_di_by_country.sql"
        db_comp_res = compare_with_postgres(
            df=df_for_db,
            table_name="ed_residents_di_by_country",
            db_name="athena",
            match_cols=["year", "country"],
            sync_cols=["area", "amount_millions", "continent"],
            tolerance=0.11,
