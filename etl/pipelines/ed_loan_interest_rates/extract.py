...
def extract_loan_interest_rates(file_path: Path) -> pd.DataFrame:
    ...
    # Load sheet 'Loans_Interest rates'
    ...
    # All columns except Date_Raw, Year, Month are rates
    rate_cols = [c for c in unique_columns if c not in ("Date_Raw", "Year", "Month")]
    
    # Use a dictionary to build the DataFrame to avoid fragmentation
    cleaned_dict = {
    ...
    for col in rate_cols:
        ...
        # Since we deduped, data[col] is guaranteed to be a Series
        series = pd.to_numeric(data[col], errors='coerce')
        
        # Drop columns that are completely empty to reduce noise
