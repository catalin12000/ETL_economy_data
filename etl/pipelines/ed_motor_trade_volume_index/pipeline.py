...
class Pipeline:
    pipeline_id = "ed_motor_trade_volume_index"
    display_name = "Motor Trade Volume Index"

    PUBLICATION_CODE = "DKT45"
    # Using substring to match even as periods in title change
    TARGET_TITLE_SUBSTRING = "04. Volume Index for Motor Trade"

    def run(self, state: Dict[str, Any]) -> Dict[str, Any]:
        out_dir = Path("data/downloads") / f"16_{self.pipeline_id}"
        out_dir.mkdir(parents=True, exist_ok=True)

        out_path = out_dir / "elstat_motor_trade_volume.xls"
