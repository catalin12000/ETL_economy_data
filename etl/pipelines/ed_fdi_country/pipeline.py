...
from .extract import extract_fdi_country


class Pipeline:
    pipeline_id = "ed_fdi_country"
    display_name = "BoG FDI Flows - Home by Country"
    MIN_DB_YEAR = 2020
    ...
    FILE_URL = "https://www.bankofgreece.gr/RelatedDocuments/BPM6_FDI_HOME_BY_COUNTRY.xls"

    def run(self, state: Dict[str, Any]) -> Dict[str, Any]:
        prefix = "10"
        out_dir = Path("data/downloads") / f"{prefix}_{self.pipeline_id}"
        out_dir.mkdir(parents=True, exist_ok=True)
        out_path = out_dir / "BPM6_FDI_HOME_BY_COUNTRY.xls"
        ...
        print("Extracting FDI country data...")
        df_all = extract_fdi_country(out_path)
        ...
        sql_path = Path(__file__).parent / "ed_fdi_country.sql"
        db_comp_res = compare_with_postgres(
            df=df_for_db,
            table_name="ed_fdi_country",
            db_name="athena",
            match_cols=["year", "country"],
            sync_cols=["area", "amount", "continent"],
            tolerance=0.11,
        ...
        target_cols = ["Year", "Country", "Area", "Amount", "Continent"]
