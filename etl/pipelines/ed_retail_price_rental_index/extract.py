...
def extract_retail_price_rental_index(price_pdf_path: Path, rent_pdf_path: Path) -> pd.DataFrame:
    price_df = _parse_index_pdf(price_pdf_path, "RETAIL PRICE INDEX").rename(
        columns={
    ...
    rent_df = _parse_index_pdf(rent_pdf_path, "RETAIL RENT INDEX").rename(
        columns={
    ...
    if df.empty:
        raise RuntimeError("No complete retail price/rent rows remained after merging both PDFs.")
    return df
